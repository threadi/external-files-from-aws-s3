...
class Configuration extends Configuration_Base {
	...
	public function get_title(): string {
		return __( 'Use only Cloudflare R2', 'external-files-from-aws-s3' );
	}
	...
	public function get_dialog_hints(): array {
		return array(
			'<p>' . __( 'This will disable all other services except Cloudflare R2.', 'external-files-from-aws-s3' ) . '<br>' . __( 'After that, you will only be able to see and use Cloudflare R2 for external sources.', 'external-files-from-aws-s3' ) . '</p>',
		);
	}
