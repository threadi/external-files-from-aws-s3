 ...
 */
class AwsS3 extends Platform_Base implements Service {
	/**
	 * The object name.
	 *
	 * @var string
	 ...
	 * @var string
	 */
	protected string $settings_sub_tab = 'eml_aws_s3';
	 ...
	 * @var string
	 */
	protected string $protocol_class_name = 'ExternalFilesFromAwsS3\Platforms\AwsS3\Protocol';
	 ...
	 * @var string
	 */
	protected string $configuration_object_name = 'ExternalFilesFromAwsS3\Platforms\AwsS3\Configuration';
	...
	public function init(): void {
		...
		// use our own hooks.
		add_filter( 'efmlawss3_service_aws_s3_hide_file', array( $this, 'prevent_not_allowed_files' ), 10, 3 );
	...
	public function get_user_settings(): array {
		$list = array(
			's3_access_key' => array(
				...
				/* translators: %1$s will be replaced by a URL. */
				'description' => sprintf( __( 'Get your access key as described <a href="%1$s" target="_blank">here (opens in a new window)</a>.', 'external-files-from-aws-s3' ), 'https://docs.aws.amazon.com/solutions/latest/data-transfer-hub/set-up-credentials-for-amazon-s3.html' ),
				'field'       => 'text',
		 ...
		 */
		return apply_filters( 'efmlawss3_service_aws_s3_user_settings', $list );
	}
	...
	public function is_file_public_available( string $file_key, S3Client $s3 ): bool {
		...
		if ( ! $public_access_allowed ) {
			// get the response.
			$response = wp_safe_remote_head( $this->get_public_url_of_file( $file_key, $this->get_fields() ) );
	...
	public function remove_authorization_header( array $args, Protocol_Base $http ): array {
		...
		return parent::remove_authorization_header( $args, $http );
	}
	...
	public function get_form_title(): string {
		...
		if ( $this->has_credentials() ) {
			return __( 'Connect to your AWS S3', 'external-files-from-aws-s3' );
		}
	...
	public function get_form_description(): string {
		...
		if ( $this->is_mode( 'global' ) ) {
			if ( $has_credentials_set && ! current_user_can( 'manage_options' ) ) {
				return __( 'An authentication JSON has already been set by an administrator in the plugin settings. Just connect for show the files.', 'external-files-from-aws-s3' );
			}

			if ( ! $has_credentials_set && ! current_user_can( 'manage_options' ) ) {
				return __( 'An authentication JSON must be set by an administrator in the plugin settings.', 'external-files-from-aws-s3' );
			}

			if ( ! $has_credentials_set ) {
				/* translators: %1$s will be replaced by a URL. */
				return sprintf( __( 'Set your authentication JSON <a href="%1$s">here</a>.', 'external-files-from-aws-s3' ), $this->get_config_url() );
			}

			/* translators: %1$s will be replaced by a URL. */
			return sprintf( __( 'Your access token is already set <a href="%1$s">here</a>. Just connect for show the files.', 'external-files-from-aws-s3' ), $this->get_config_url() );
		}
		...
		if ( $this->is_mode( 'user' ) ) {
			if ( ! $has_credentials_set ) {
				/* translators: %1$s will be replaced by a URL. */
				return sprintf( __( 'Set your authentication JSON <a href="%1$s">in your profile</a>.', 'external-files-from-aws-s3' ), $this->get_config_url() );
			}

			/* translators: %1$s will be replaced by a URL. */
			return sprintf( __( 'Your authentication JSON is already set <a href="%1$s">in your profile</a>. Just connect for show the files.', 'external-files-from-aws-s3' ), $this->get_config_url() );
		}
	...
	private function get_field_values(): array {
		...
		$values = array(
			...
			'secret_key' => '',
			'bucket'     => '',
			'region'     => get_option( 'eml_s3_region', '' ),
		);
	...
	public function get_requested_url( string $url, array $fields ): string {
		return str_replace( $this->get_url_mark( $fields['bucket']['value'] ), '', $url );
	}
}
