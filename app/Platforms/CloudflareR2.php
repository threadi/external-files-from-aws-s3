 ...
 */
class CloudflareR2 extends Platform_Base implements Service {
	/**
	 * The object name.
	 *
	 * @var string
	 ...
	 * @var string
	 */
	protected string $protocol_class_name = 'ExternalFilesFromAwsS3\Platforms\CloudflareR2\Protocol';
	 ...
	 * @var string
	 */
	protected string $configuration_object_name = 'ExternalFilesFromAwsS3\Platforms\CloudflareR2\Configuration';
	 ...
	 * @var string
	 */
	protected string $settings_sub_tab = 'eml_cloudflare_r2';
	...
	public function init(): void {
		...
		// set title.
		$this->title = __( 'Choose file(s) from your Cloudflare R2 server', 'external-files-from-aws-s3' ); // @phpstan-ignore property.notFound

		// use our own hooks.
		add_filter( 'efmlawss3_service_cloudflare_r2_hide_file', array( $this, 'prevent_not_allowed_files' ), 10, 3 );
		add_filter( 'efmlawss3_cloudflare_r2_query_params', array( $this, 'change_file_query' ) );
	...
	public function do_login( string $directory ): bool {
		...
		if ( empty( $this->get_fields() ) ) {
			// create error object.
			$error = new WP_Error();
			$error->add( 'efml_service_cloudflare_r2', __( 'No credentials set for this Cloudflare R2 connection!', 'external-files-from-aws-s3' ) );
		...
		try {
			...
			$error->add( 'efml_service_cloudflare_r2', sprintf( __( 'Credentials and/or bucket are not valid. Cloudflare R2 returns with HTTP-Status %1$d!', 'external-files-from-aws-s3' ), $e->getStatusCode() ) );
			...
			Log::get_instance()->create( sprintf( __( 'Credentials and/or bucket are not valid. Cloudflare R2 returns with HTTP-Status %1$d! Error:', 'external-files-from-aws-s3' ), $e->getStatusCode() ) . ' <code>' . $e->getMessage() . '</code>', '', 'error' );
	...
	/**
	 * Enable WP CLI for Cloudflare R2 tasks.
	 *
	 * @return void
	 */
	 ...
	 * @return void
	 */
	public function init_cloudflare_r2(): void {
		// bail if user has no capability for this service.
		...
		if ( defined( 'EFML_ACTIVATION_RUNNING' ) || 'global' === get_option( 'eml_' . $this->get_name() . '_credentials_vault' ) ) {
			// add setting.
			$setting = $settings_obj->add_setting( 'eml_cloudflare_r2_account_id' );
			...
			$field->set_placeholder( __( 'The account ID', 'external-files-from-aws-s3' ) );
			$setting->set_field( $field );

			// add setting.
			$setting = $settings_obj->add_setting( 'eml_cloudflare_r2_eu' );
			$setting->set_section( $section );
			$setting->set_autoload( false );
			...
			// add setting.
			$setting = $settings_obj->add_setting( 'eml_cloudflare_r2_access_key' );
			$setting->set_section( $section );
			$setting->set_autoload( false );
			$setting->set_type( 'string' );
			$setting->set_default( '' );
			...
			// add setting.
			$setting = $settings_obj->add_setting( 'eml_cloudflare_r2_secret_key' );
			$setting->set_section( $section );
			$setting->set_autoload( false );
			$setting->set_type( 'string' );
			$setting->set_default( '' );
			...
			// add setting.
			$setting = $settings_obj->add_setting( 'eml_cloudflare_r2_bucket' );
			$setting->set_section( $section );
			$setting->set_autoload( false );
			$setting->set_type( 'string' );
			$setting->set_default( '' );
		...
		if ( 'user' === get_option( 'eml_' . $this->get_name() . '_credentials_vault' ) ) {
			$setting = $settings_obj->add_setting( 'eml_cloudflare_r2_credential_location_hint' );
			$setting->set_section( $section );
		...
		$setting = $settings_obj->add_setting( 'eml_cloudflare_r2_import_limit' );
		$setting->set_section( $section );
		$setting->set_type( 'integer' );
	...
	public function get_user_settings(): array {
		...
		/**
		 * Filter the list of possible user settings for Cloudflare R2.
		 *
		 * @since 1.0.0 Available since 1.0.0.
		 * @param array<string,mixed> $list The list of settings.
		 */
		return apply_filters( 'efmlawss3_service_cloudflare_r2_user_settings', $list );
	}

	/**
	 * Return the URL mark, which identifies Cloudflare R2 URLs within this plugin.
	 *
	 * @return string
	 */
	public function get_url_mark(): string {
		return 'https://dash.cloudflare.com/' . $this->get_fields()['account_id']['value'] . '/';
	}
	...
	public function get_form_title(): string {
		...
		if ( $this->has_credentials() ) {
			return __( 'Connect to your Cloudflare R2', 'external-files-from-aws-s3' );
		}
	...
	public function get_form_description(): string {
		...
		/* translators: %1$s will be replaced by a URL. */
		return sprintf( __( 'Enter your Cloudflare R2 credentials in this form. How to get them for Cloudflare R2 is described <a href="%1$s">here</a>.', 'external-files-from-aws-s3' ), 'https://developers.cloudflare.com/r2/api/tokens/' );
	}
	...
	/**
	 * Allow to use Cloudflare R2 URLs.
	 *
	...
	/**
	 * Set the real mime type for Cloudflare R2 URLs.
	 *
	...
	public function get_s3_client(): S3Client {
		...
		$endpoint_url = 'https://' . $fields['account_id']['value'] . '.r2.cloudflarestorage.com';
		if ( $fields['eu']['value'] ) {
			$endpoint_url = 'https://' . $fields['account_id']['value'] . '.eu.r2.cloudflarestorage.com';
		}
