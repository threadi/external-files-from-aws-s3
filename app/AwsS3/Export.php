...
class Export extends Export_Base {
	...
	public function export_file( int $attachment_id, string $target, array $credentials ): string|bool {
		...
		if ( ! str_starts_with( $target, $s3_obj->get_directory() ) ) {
			// log this event.
			Log::get_instance()->create( __( 'Given path is not a AWS S3-URL.', 'external-files-from-aws-s3' ), $target, 'error' );
		...
		Log::get_instance()->create( sprintf( __( 'Exporting attachment ID %1$s to AWS S3 bucket %2$s.', 'external-files-from-aws-s3' ), '<em>' . $attachment_id . '</em>', '<em>' . $s3_obj->get_bucket_name() . '</em>' ), $target, 'info', 2 );
		...
		if ( ! $s3_obj->is_file_public_available( $key, $s3_client ) ) {
			// log this event.
			Log::get_instance()->create( __( 'File would not be public available on your AWS S3 bucket. Check the settings in the bucket to use the export of files.', 'external-files-from-aws-s3' ), $target, 'error' );
	...
	public function delete_exported_file( string $url, array $credentials, int $attachment_id ): bool {
		...
		if ( ! str_starts_with( $url, $s3_obj->get_directory() ) ) {
			// log this event.
			Log::get_instance()->create( __( 'Given path is not a AWS S3-URL.', 'external-files-from-aws-s3' ), $url, 'error' );
